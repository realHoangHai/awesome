...
type (
	...
	Server interface {
		// HealthServer implements grpc_health_v1.HealthServer for general health check and
		// load balancing according to gRPC protocol.
		grpc_health_v1.HealthServer
		// Handler implements http.Handler Check API via HTTP.
		http.Handler
...
func (s *MServer) checkAll() {
	...
	for service, check := range s.checks {
		...
		s.server.SetServingStatus(service, state)
	}
